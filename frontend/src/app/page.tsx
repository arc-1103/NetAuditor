...
import { approveRemediation, generateRemediations, generateReport, getAudit, login, reportUrl, uploadConfig, USE_MOCK } from "../lib/api";
...
function FindingPanel({ finding, remediation, onDecision, busy }: { finding: Finding; remediation?: Remediation; onDecision: (approved: boolean) => void; busy: boolean }) {
  ...
  return <aside className="drawer">
    ...
    <h2>{finding.title}</h2>
    <section><h3>Observed evidence</h3><pre className="evidence">{finding.evidence}</pre>{finding.source_lines?.length && <p className="source">Source line{finding.source_lines.length > 1 ? "s" : ""}: {finding.source_lines.join(", ")}</p>}</section>
    <section><h3>Why it matters</h3><p className="muted">This configuration weakens the management plane and can increase unauthorized access or lateral-movement risk.</p>{finding.blast_radius?.length ? <p className="blast">Potential blast radius · {finding.blast_radius.join(" · ")}</p> : null}</section>
...
export default function Home() {
  ...
  async function handleFile(file?: File) {
    ...
    try { const uploaded = await uploadConfig(file, token); const run = await getAudit(uploaded.job_id, token); setAudit({ ...run, original_filename: USE_MOCK ? file.name : run.original_filename }); setSelected(run.findings[0] || null); setNotice("Audit completed. Every verdict below was produced by version-controlled policy."); }
  ...
  async function report() {
    ...
    try { await generateReport(audit.id, token); setNotice(USE_MOCK ? "Report generated. Real deployment provides signed PDF, JSON and CEF downloads." : "Evidence report generated and ready to download."); if (!USE_MOCK) window.open(reportUrl(audit.id, "preview"), "_blank"); }
  ...
  return <main className="app-shell">
    ...
    <div className="workspace"><header><div><p className="eyebrow">SECURITY OPERATIONS</p><h1>Audit workspace</h1></div><div className="header-actions"><span className="airgap">● AIR-GAP READY</span>{audit && <button className="secondary" onClick={report} disabled={!!busy}>{busy === "report" ? "Building report…" : "Export evidence"}</button>}</div></header>
