...
must never fail or block the remediation operation that triggered it, so
failures are logged and swallowed here, never raised, and a short timeout
keeps a hung endpoint from blocking the caller.

Kept as its own small module per service rather than a shared package — see
services/compliance/app/webhooks.py, which is identical for the same
reason SEVERITY_WEIGHTS is duplicated in services/reporting.
"""
...
logger = logging.getLogger("netaudit.remediation.webhooks")
